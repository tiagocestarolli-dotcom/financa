# PROMPT-MESTRE — Projeto Finança · v21

> **Como usar este documento:** este é o documento de **continuidade e memória** do projeto Finança. Ele NÃO é o que roda o app — quem roda é o `index.html`. A função dele é permitir que um novo chat com o Claude retome o projeto com contexto total, sabendo o estado atual, as decisões já tomadas e o que falta. Mantenha-o aqui no **Knowledge do projeto** (é o que o Claude lê automaticamente); o `index.html` mora no **GitHub** (é de lá que o app faz deploy). Não é preciso manter este prompt sincronizado no GitHub.
>
> Ao retomar num chat novo: o Claude lê este documento + consulta o `index.html` anexado. Comece pela seção **"LEIA PRIMEIRO"** abaixo — ela dá o essencial; o resto é referência pra consultar conforme a necessidade.

---

## 🚀 LEIA PRIMEIRO — Estado do projeto em 30 segundos

- **O que é:** app pessoal/familiar de controle financeiro (single-file `index.html` — CSS+HTML+vanilla JS, Firebase Auth+Firestore+Hosting). Domínio: família (Casa·Marina·Heitor·Juliana) + Tiago isolado por design.
- **Versão atual:** `v0.33.0 · Fatia 5c.5`. Rodapé do Config é a **fonte única** da versão (`#appVersionTag`).
- **O que o app já faz:** registra entradas/gastos (fixos, parcelas, investimentos); importa **faturas de cartão** em PDF (parsers Itaú Visa Infinite, Sicoob, BTG — todos prontos); importa **extrato bancário** (Itaú **e Sicoob** prontos; BTG pendente — com undo em lote, dedup em 3 níveis, classificação inline com nome/subcategoria/grupo/fixo); paga faturas (Etapa 4); gera **4 relatórios** (Gastos&Saídas · Fluxo de Caixa · Meus Gastos · Detalhado) + 2 saídas PDF (PDF rápido da tela + **Relatório Elegante** compartilhável, agora com gastos por categoria e lista cronológica controláveis).
- **Onde paramos:** **Etapa 5 FECHADA** (21/maio/2026). A unificação de relatórios avançou bastante: o **Relatório Elegante** agora cobre Rel 1, Rel 2 e Rel 3 com detalhamentos ricos — fluxo de caixa lançamento-a-lançamento (Rel 2), gastos detalhados por categoria/membro e lista cronológica (Rel 1/3), tudo com interruptores numa caixa única. O parser de **extrato Sicoob** entrou (conciliação de saldo fechou em R$ 0,00).
- **Próximas opções (backlog, a decidir com o Tiago):** (1) **dar Relatório Elegante ao Rel 4** e **aposentar o PDF rápido** (passo final da unificação — exige resolver o Rel 4 antes); (2) **caixa de toggles própria no Rel 3** (hoje obedece os interruptores ligados no Rel 1); (3) **Etapa 5b.4** — parser de *extrato de conta* BTG (BLOQUEADA: precisa de extrato de conta corrente real do BTG); (4) **Etapa 6 — BLUEPRINT-FINANCA** — generalização pra reconstruir o app / adaptar pra empresa (precisará de **dashboard ao vivo** além dos relatórios formais — ver nota de design abaixo).
- **Como o Tiago trabalha:** médico/gestor, não-dev. Edita pelo iPhone (Safari anônimo). Claude entrega `index-vX.Y.Z.html` por download → Tiago renomeia pra `index.html` e sobe no GitHub → Action faz deploy em 1-2 min → testa com `?v=` na URL pra furar cache.
- **Ritual de entrega (não-negociável):** antes de entregar qualquer `index.html`, rodar os 5 checks (sintaxe JS, zero NBSP, zero backticks vazados, versão única no rodapé, nome de arquivo versionado) e, ao concluir fatia, **atualizar este prompt mestre junto** (foi não-cumprido entre v0.25 e v0.27, o que gerou a defasagem corrigida nesta v20).

---

## ✅ STATUS ATUAL — ETAPA 5 FECHADA · v0.33.0

**Última versão entregue:** `v0.33.0 · Fatia 5c.5 — lista cronológica no Relatório Elegante (Rel 1 e Rel 3)` (rodapé do Config).

**Sessão de 21/maio/2026 — Unificação dos relatórios + parser de extrato Sicoob (5 fatias: v0.29.0 → v0.33.0):**

Arco da sessão: (a) destravar a importação de **extrato Sicoob** com material real; (b) avançar a **UNIFICAÇÃO dos relatórios** rumo a um botão só — fazer o Relatório Elegante crescer até cobrir tudo, pra eventualmente aposentar o "PDF rápido". Ao fim, a **Etapa 5 foi declarada FECHADA**.

**(5b.3) v0.29.0 — Parser de extrato de conta Sicoob.** `parserExtratoSicoob` + `_sicoobDataPdfParaISO`, plugados no dispatcher `parseExtrato` (case 'sicoob'). Layout Sicoob: data DD/MM (ano vem do PERÍODO no header), valor BR com sufixo colado **D**=débito/saída, **C**=crédito/entrada, **\***=saldo (ignora); descrições multi-linha (detalhe nas linhas seguintes — estabelecimento/CPF/DOC); seção RESUMO+LANÇAMENTOS FUTUROS cortada (corte em `/^RESUMO/`). Classificação estendida em `classificarLancamentosExtrato` com bloco Sicoob guardado por `if (l.historico)` (campo que só o parser Sicoob preenche — Itaú não tem, segue intacto): pagamento de fatura = DÉB.CONV.DEM.EMPRES + DOC MASTERCARD (anti-duplicidade, casa com `_matchPagamentoFatura`); créditos → ENTRADA_RENDA; tarifas (PACOTE SERVI/CONV SEGUROS) → SAIDA_TARIFA; resto → SAIDA_CONSUMO (Tiago classifica). **Audit de ouro:** a conciliação de saldo (anterior + créditos − débitos = final) fechou em **R$ 0,00** no extrato de maio (32 lançamentos). Telemetria em `window.__extratoDebug.auditSaldo`. Validado por transplante (funções extraídas do index e rodadas contra o texto real do PDF) + não-regressão Itaú. BTG (5b.4) segue bloqueado (falta extrato de conta real do BTG).

**(5c.2) v0.30.0 — Fluxo de caixa detalhado no Relatório Elegante (Rel 2).** Seção "🧾 Fluxo detalhado · lançamento a lançamento": lista cronológica de entradas (verde +), saídas (vermelho −) e investimentos (azul), com rodapé que reconcilia com a capa (Entradas / Saídas+Invest / Saldo). Coletada em `_dadosRel2` (`listaFluxo`, reusando `state.entradas` + `gastosCaixa` — reconcilia por construção, pois soma(gastosCaixa) = saidas + invest). Helpers `_secaoFluxoDetalhado` + `_dataCurta`. Cores inline (vencem a especificidade do `.tg-val`).

**(5c.3) v0.31.0 — Gastos detalhados por categoria no Relatório Elegante (Rel 1).** Seção "🧷 Gastos detalhados por categoria": para cada membro (ordenado por total) → **total no topo** → **subcategoria com subtotal** → **itens individuais** (nome, valor, data discreta). Compacto, fonte menor. Coletado em `_dadosRel1` (`detalhePorCategoria`, mesma base `gastosPeriodo` — reconcilia: total membro = Σsubtotais = Σitens). Helper `_secaoDetalhePorCategoria`. CSS `.dpc-*`. Entra após os gráficos. Vale também pro Rel 3 (usa `_gerarRel1`). **Granularidade confirmada pelo Tiago** e definida como o **formato principal** do relatório (ver decisões).

**(5c.4) v0.32.0 — Caixa de opções do Rel 1 + toggle de gastos por categoria.** Os 3 controles do Rel 1 agrupados numa **caixa única empilhada** (`.rel-opts`, reusa o visual do toggle Tiago): incluir Tiago / incluir gastos detalhados por categoria (NOVO) / incluir lista detalhada. Nova preferência `relPdfDetalheCategoria` (localStorage `financa.relPdfDetalheCategoria`, **default LIGADO** — Tiago desliga nos períodos longos pra não encher de itens) controla a seção 5c.3 no `_gerarRel1`. O apêndice "lista detalhada" saiu da barra no Rel 1 (foi pra caixa, `mostraOptApendice` agora só `rel3`); segue na barra no Rel 3.

**(5c.5) v0.33.0 — Lista cronológica no Relatório Elegante (Rel 1 e Rel 3).** Seção "🧾 Lançamentos detalhados · ordem cronológica": saídas (consumo + invest) por data, total no rodapé. Coletada em `_dadosRel1` (`listaCronologica`). Helper `_secaoCronologicoRel1` (reusa estilo `.fx-*` da 5c.2). Controlada pelo toggle "lista detalhada" (`relPdfApendice`) — que agora vale TANTO no PDF rápido QUANTO no Elegante. **Coerência alcançada:** o que o Tiago liga na caixa é o que sai no relatório elegante. Default do cronológico: **DESLIGADO** (é ferramenta de conferência/localização, não o formato principal).

**Etapa 5 FECHADA (decisão do Tiago):** relatórios elegantes (Rel 1/2/3) com detalhamentos controláveis + extratos importando = coração da etapa, pronto. Refinamentos viraram backlog (ver §10): Rel 4 elegante, aposentar o PDF rápido, caixa de toggles própria no Rel 3. **"Levar detalhamentos pra tela" foi DISPENSADO** — o app já tem o extrato do mês (toggle) e o drill nas categorias/subcategorias; duplicar não acrescentaria.

---

## ✅ Sessão anterior (20–21/maio/2026) — Fatia 5e.1 (paridade do picker de extrato) · v0.28.0 + consolidação v19→v20

**Versão entregue:** `v0.28.0 · Fatia 5e.1 — classificar extrato com grupo novo + marcar como fixo` (rodapé do Config).

**Fatia 5e.1 (v0.28.0) + consolidação deste prompt mestre (v19 → v20):**

Dor-raiz: o Tiago relatou em uso real que, ao importar extrato, a tela de classificação NÃO deixava criar grupo nem marcar gasto como fixo (e ele não enxergava os campos de nome/criar-subcategoria, que JÁ existiam no código v0.27.0 — provável cache de PWA no iPhone). Auditoria do código confirmou: o picker do extrato (`_rexAbrirPickerClassificacao`) tinha nome + criar-subcategoria-inline, mas o botão "✎ editar" só aparece em saídas (`SAIDA_CONSUMO`/`SAIDA_TARIFA`), e faltavam "criar grupo novo" e "marcar como fixo" — recursos que o picker da FATURA (`openItemAtribuicaoIndividual`) já tinha. **Correção de rota importante:** a nota do prompt mestre que dizia "estender nome+nova-subcat ao picker de FATURA" estava ERRADA — a fatura já tinha tudo; o gap real era no EXTRATO, em grupo+fixo.

**(5e.1) v0.28.0 — paridade do picker de classificação do extrato.** No `_rexAbrirPickerClassificacao`: (a) seção "grupo" agora aparece SEMPRE (antes sumia quando a categoria não tinha grupos, impedindo criar o primeiro), com botão "+ novo" que cria grupo INLINE (estado `criandoGrupo`, mesmo padrão do `criandoSub` da subcategoria — evita modal `askText` empilhado, que ficaria ATRÁS do picker já que ele é z-index 250 e o askText 200); (b) novo toggle "🔁 marcar como fixo" (reusa `.switch`/`.sl`, fica verde `--flow-in` quando ligado). Estado novo: `pickerFixo`, `criandoGrupo`. Save grava `classifManualFixo`. `aplicarLancamentosExtratoImportados` (ramo `if (l.classifManual)`) agora faz `isFixo = l.classifManualFixo === true` e passa esse `isFixo` pro `aprenderAlias` (antes hardcoded `false`). Compatibilidade retroativa: lançamentos antigos sem o campo → `=== true` resolve pra `false`, sem quebrar. Validação: 5 checks da skill de entrega + verificação cruzada de IDs↔handlers + simulação Node 3/3 (marcou fixo+grupo / não marcou / lançamento antigo sem o campo). Schema persistido sem mudanças (os `classifManual*` vivem no `revisaoExtratoState` temporário).

**Consolidação do prompt mestre (v19 → v20):** o V19 estava internamente desencontrado — topo em v0.27.0 mas histórico parado na v18 (v0.25.0), roadmap com pendência errada, carimbos de schema/pipeline em v0.15.0. Esta v20 sincroniza tudo, adiciona o bloco "LEIA PRIMEIRO" no topo (re-inteiração barata de chats novos), registra v0.26/v0.27/v0.28 no histórico e corrige o roadmap. **Lição de processo:** atualizar o prompt mestre a CADA fatia (a skill `financa-entregar-fatia` já manda); deixar acumular gera defasagem cara em chats futuros. E: ao bater documentação contra comportamento relatado, **o código é a fonte de verdade** — a nota sobre a fatura estava errada e só o código revelou.

**Decisões de escopo abertas (Tiago pode mudar):** subcategoria/grupo novos nascem EXCLUSIVOS do membro (não universais); renomear subcategoria/grupo existente continua só na tela de Config (não foi trazido pro picker — candidato a fatia futura).

**Próximo: Fatia 5c.2 (append de fluxo no Rel 2) OU Etapa 5b.3/5b.4 (parsers de extrato Sicoob/BTG — requerem extrato de conta real) OU Etapa 6 (BLUEPRINT-FINANCA) OU renomear subcat/grupo nos pickers.**

---

## ✅ Sessão anterior (20/maio/2026, noite) — Fatias 5d + 5e (relatório elegante + extrato cronológico) · v0.27.0

**Versão entregue:** `v0.27.0 · Fatia 5e — extrato cronológico + classificar com nome/subcategoria`.

**Sessão de 20/maio/2026 (noite) — Fatias 5d e 5e, duas versões (v0.26.0 → v0.27.0):**

Duas entregas em cima do mesmo tema-raiz: dar ao Tiago ferramentas de **conferência** (bater o app contra documentos do banco) e de **compartilhamento** (relatório digno de mostrar pra Juliana / um auditor).

**(5d) v0.26.0 — Relatório Elegante (PDF compartilhável).** Rota de exportação SEPARADA do botão "📄 PDF" da Fatia 5.5 (que imprime a própria tela via `window.print()`). Novo namespace `window.relatorioElegante`: gera um documento HTML AUTÔNOMO (Montserrat via @import, capa navy, gráficos SVG em colunas, layout A4) aberto em nova aba com barra de ferramentas (imprimir/salvar PDF + baixar HTML) e fallback de download quando o popup é bloqueado no iOS. Abrange Rel 1 (Gastos & Saídas), Rel 3 (Meus Gastos · Tiago — mesmo motor com `cats=['tiago']`) e Rel 2 (Fluxo de Caixa). Rel 4 ficou de fora (já tem apêndice cronológico próprio + filtros que tornariam o documento ambíguo). Botão "✦ Relatório" (navy, `data-rel-elegante`) ao lado do "📄 PDF" só aparece em rel1/rel2/rel3; tooltip do botão antigo virou "PDF rápido — imprime a tela atual". CHAVE: o módulo chama os MESMOS motores de cálculo das telas (`_relGastoSaida`, `_relGastoConta`, `_isInvestRuntime`, `relCalcCompromissos`, `rel2Calc*`), então tudo reconcilia por construção (total da capa == consumo + investimento == soma por membro == soma por subcategoria == soma dos meses do histórico). Decisão de anti-padrão: SEM taxa de poupança / runway / % comprometido — o reference `report-sections.md` da skill os inclui, mas a `SKILL.md` os proíbe; **a SKILL prevalece sobre o reference file**. Validação: 33/33 testes de reconciliação em Node + 4 checks da skill de entrega.

**(5e) v0.27.0 — Extrato cronológico + classificar com nome/subcategoria.** Dois pedidos do Tiago, mesma dor-raiz (conferir extrato bancário contra lançamentos):
- **Extrato cronológico na tela** (não só no relatório): botão "🧾 Ver extrato do mês" no fim das views Família e Tiago (`data-extrato`, bind único no init junto do `data-compromissos`). `openExtratoSheet(escopo)` abre sheet com month-nav (‹›), total, contagem, BUSCA livre (nome/subcategoria/membro/valor — casa "47,90" e "47.90") e a lista via `_relListaCronologicaHtml` reusado (cada linha já mostra data, categoria›subcat, forma de pgto, badges ❓a classificar / 📥extrato / 📄fatura; tocar abre `openGastoForm`). Família achata os 4 membros numa lista só (resolve o pula-pula entre subtabs); Tiago mostra só `categoria==='tiago'`. Escopo: gastos do mês corrente da view (`famCurrentYM`/`tiagoCurrentYM`), exclui só `isAjuste`. `_extratoAtualizarLista()` atualiza só lista+total (preserva foco da busca).
- **Picker de classificação do extrato com nome + nova subcategoria** (`_rexAbrirPickerClassificacao`): campo "nome / identificação" no topo (default = descrição crua do extrato, editável) → vira o `nome` do gasto; botão "+ nova" cria subcategoria INLINE (modo `criandoSub`, evita modal empilhado — o overlay do picker é z-index 250 e o `askText` é 200, ficaria ATRÁS), salva como exclusiva do membro (`state.subcatsExclusivas[cat]`). `aplicarLancamentosExtratoImportados` agora usa o nome amigável como `nome` do gasto E o ensina ao alias via o parâmetro `identificacao` de `aprenderAlias` (campo que já existia no schema mas nunca era alimentado no fluxo de extrato); o ramo de auto-classificação por alias passou a aplicar `alias.identificacao` como nome. Validação: busca 9/9 em Node + 4 checks da skill.

**Decisões de escopo abertas (Tiago pode mudar):** subcategoria nova nasce EXCLUSIVA do membro (não universal); extrato lista só GASTOS (não entradas) do mês corrente.

**Próximo: Fatia 5c.2 (append de fluxo no Rel 2 — entradas/saídas/invest) OU Etapa 5b.3 (parser de extrato Sicoob — requer extrato de conta real) OU Etapa 6 (BLUEPRINT-FINANCA / generalização comercial) OU estender nome+nova-subcat ao picker de FATURA (`renderRevisaoItem` — hoje só o extrato tem).**

---

## Sessão anterior (20/maio/2026) — Fatia 5c.1 (filtros no Detalhe + lista detalhada nos PDFs) · v0.25.0

**Última versão entregue:** `v0.25.0 · Fatia 5c.1 — filtros no Detalhe + lista no PDF` (rodapé do Config).

**Sessão de 20/maio/2026 — Fatia 5c.1 (v0.25.0):**

Escolhida entre 3 caminhos (5c.1 / Etapa 5b.3 Sicoob / Etapa 6 BLUEPRINT). 5c.1 venceu por continuidade (Rel 4 recém-entregue, contexto fresco), alinhamento com "auditoria fiscal profissional" e risco baixo (schema sem mudanças, reuso máximo). 5b.3 depende de extrato de conta Sicoob em mãos; Etapa 6 seria generalização prematura.

**(5c.1.a) Filtros no Relatório Detalhado (Rel 4)** — `rel4State` ganha `filtroSubcats[]` e `filtroGrupos[]`. Helpers novos: `_rel4AplicarFiltros(gastos)` (interseção subcat ∩ grupo, vazio = passa tudo) e `_rel4Catalogo(gastosBase)` (computa as opções de chip a partir da BASE não-filtrada — separar "universo" de "recorte"). `_rel4HeaderHtml` ganha 3º parâmetro `catalogo` e renderiza a seção "filtrar" com chips multi-seleção (subcategoria sempre que há ≥2; grupo só se houver grupos na base). Cartão de resumo e lista recalculam sobre o filtrado. Botão "limpar filtros". Eventos: toggle por chip, limpar, e reset de filtros ao trocar escopo (mantém ao navegar meses). `_atualizarPrintMeta` (rel4) põe o recorte no título do PDF ("Heitor · Escola, Esporte").

**(5c.1.b) Lista detalhada anexável aos PDFs do Rel 1 e Rel 3** — toggle "📎 incluir lista detalhada no PDF" no header (só Rel 1/Rel 3), preferência em `localStorage` (`financa.relPdfApendice`), variável global `relPdfApendice`. Render cronológico extraído de `_renderRel4Inner` para `_relListaCronologicaHtml(gastos)` — recebe a lista já filtrada e só renderiza; reusado por tela (Rel 4) e PDF. `_relApendiceGastos(viewId)` coleta os gastos com o MESMO conjunto+filtro do hero do relatório-mãe (Rel 1: `relCategoriasIncluidas()` respeitando toggle Tiago; Rel 3: `REL3_CATS_TIAGO`; ambos `_relGastoSaida`), respeitando o PERÍODO selecionado via `relYmsHistorico`. `_montarApendicePDF`/`_limparApendicePDF` injetam/removem o bloco no `#relApendice` — invisível em tela (CSS `.rel-apendice{display:none}`), revelado em `@media print` com `page-break-before:always` quando tem `.tem-conteudo`; limpeza no `window.addEventListener('afterprint', …)`. Cabeçalho do apêndice declara período + count + total (reconcilia com o hero por construção).

**Decisão de escopo (muda o plano original "Rel 1/2/3"):** o append do **Rel 2 ficou de fora** — é regime de caixa, inclui ENTRADAS (sem categoria/membro) e usa cache de faturas pagas; estrutura diferente que pede renderer próprio de "extrato de fluxo". Isolado como **Fatia 5c.2** pra não misturar risco no relatório mais delicado. Rel 1/Rel 3/Rel 4 são a mesma estrutura → saíram com reuso limpo.

**Bônus:** ativada a variável CSS `--chip-color` (definida inline nos chips de escopo desde v0.24.0 mas sem nenhuma regra consumindo — dead code). Agora `.rel-det-chip.active` pinta o escopo selecionado com a cor do membro.

**Validação:** 4 checks da skill de entrega (sintaxe JS, 0 NBSP, 0 backtick, versão única) + simulação Node 21/21 cobrindo filtros (subcat, grupo, interseção, interseção vazia), exclusão de técnicos, e o invariante **total do apêndice == total do hero** com Tiago OFF (R$ 2.000), Tiago ON (R$ 4.550) e período 3m (R$ 3.250). Schema sem mudanças. 6 lições novas em NOTAS-COMERCIAIS (itens 64–69). 69 itens no total.

**Próximo: Fatia 5c.2 (append de fluxo no Rel 2 — entradas/saídas/invest com renderer próprio) OU Etapa 5b.3 (parser de extrato Sicoob — requer extrato de conta real) OU Etapa 6 (BLUEPRINT-FINANCA / generalização comercial).**

---

## Sessão anterior (19–20/maio/2026) — Etapa 5b (importação de extrato — Itaú) + Fatia 5c (Relatório Detalhado) — 4 versões (v0.23.0 → v0.24.0):

Sessão longa e iterativa atravessando 2 etapas distintas. A primeira (5b) construiu o pipeline de importação de extrato bancário, com 3 patches sucessivos resolvendo problemas reais reportados pelo usuário em uso real. A segunda (5c) inaugurou um novo tipo de relatório complementar aos existentes — lista cronológica de gastos por escopo, otimizada para auditoria.

**(A) v0.23.0 — Etapa 5b · Fatias 5b.1 + 5b.2 bundled: pipeline + parser Itaú**

Decisão arquitetural: **bundling** das fatias 5b.1 (infraestrutura + tela de revisão) e 5b.2 (parser Itaú) numa única versão. O extrato Itaú do mês atual já estava em mãos, então fazia mais sentido entregar end-to-end testável do que entregar 5b.1 com mock e depois 5b.2 separado.


Implementação:
- **Arquitetura paralela ao parser de fatura** — não substitui, coexiste. Reusa `extrairTextoDoPDF()` (PDF.js), `parseValor()`, sheet component, sistema de aliases.
- **Funções novas:** `detectarBancoExtrato()`, `parseExtrato()` (dispatcher), `parserExtratoItau()`, `classificarLancamentosExtrato()`, `dedupExtrato()`, `openRevisaoExtratoSheet()`, `aplicarLancamentosExtratoImportados()`.
- **Classificação automática em 5 tipos:** `PAGAMENTO_FATURA` (match com `isPagamentoFatura` existente — não duplica), `ENTRADA_RENDA`, `ENTRADA_RENDIMENTO`, `SAIDA_CONSUMO`, `SAIDA_TARIFA`, `IGNORAR` (SALDO DO DIA, REND PAGO APLIC).
- **Botão "Importar extrato"** adicionado na tela de detalhe da conta (junto com Ajustar saldo + Editar conta).
- **Tela de revisão em lote** com filtros por tipo, checkboxes incluir/ignorar, contador dinâmico no botão de aplicar.
- **Parser Itaú validado contra extrato real (5 páginas, 87 lançamentos):** identifica 8 tipos de linha — `SALDO DO DIA` (ignora), `REND PAGO APLIC AUT` (ignora), `PERS INFINIT XXXX-XXXX` (pagamento fatura Visa Infinite), `ITAU BLACK XXXX-XXXX` (pagamento fatura MC Black), `PIX TRANSF [nome]`, `PIX QRS [nome]`, `PAY [abrev] DD/MM`, `PAG BOLETO [nome]`, `DA [operadora]`, `TED [código]`, `SEGURO CARTAO`, `SEG TRANSACAO PROTEGIDA`. 11/11 checks passaram em teste isolado.
- **Single-source-of-truth da versão:** rodapé do Config ganha `id="appVersionTag"`, novo helper `_appVersionShort()` lê o texto e retorna forma curta pro PDF (elimina string duplicada de versão hardcoded no JS — antes o check 4 da skill `financa-entregar-fatia` ficava reclamando de 2 ocorrências; agora vive em UM lugar só).

**Falha exposta em uso real (B):** Tiago importou e clicou em "aplicar" sem revisar lançamento por lançamento. Resultado: todos os ~50 gastos do extrato entraram como `categoria: 'tiago', subcategoria: 'a classificar'`, e o dedup (frouxo) não pegou as duplicatas óbvias. Sem mecanismo de undo, era preciso deletar cada um manualmente. 3 falhas de design da v0.23.0 identificadas: (1) sem undo, (2) default "Tiago" para tudo, (3) sistema de aliases não foi reaproveitado no extrato (parser de fatura usa, mas parser de extrato ignorou).

**(B) v0.23.1 — Sistema de lote de importação + undo + pré-classificação por alias**

Resposta direta às 3 falhas:

- **Schema novo:** `state.lotesImportacao: []` — array de objetos `{id, tipo, contaId, contaNome, dataImport, descricao, gastoIds[], entradaIds[], quantidade, isRecovery?}`. Cada gasto/entrada importado leva `loteImportacaoId`. FK reversa pro undo encontrar todos os records do lote.
- **Migração retroativa idempotente** rodada no boot (`migrarOrfaosExtratoEmLoteRecovery()`): qualquer gasto/entrada com `origem: 'extrato'` que não tem `loteImportacaoId` é agrupado num lote único de "🛟 Recuperação" com timestamp e flag `isRecovery: true`. Isso permitiu desfazer a importação bagunçada da v0.23.0 sem perder os gastos manuais.
- **Função `desfazerLoteImportacao(loteId)`:** filtra `state.gastos` e `state.entradas` removendo os tagueados com o lote, depois remove o lote do histórico. Toast informa quantidade desfeita.
- **Nova UI no Config:** item "📥 Histórico de importações" abre sheet com lista cronológica de lotes; cada um com data, descrição, contagem, e botão "desfazer". Confirmação modal antes do undo via `askConfirm()`.
- **Confirmação modal antes de aplicar** importação: "Vou criar X saídas + Y entradas. Confirma?" — friction proposital pra evitar o "ok sem revisar" da v0.23.0.
- **Pré-classificação por aliasItems:** antes de mostrar a tela de revisão, o parser passa cada `lanc.descricaoOriginal` por `buscarAliasParaDescricao()` (mesmo helper que o parser de fatura usa). Se acha match, pré-preenche categoria/subcategoria/grupoId/isFixo. Próximas importações do mesmo extrato vêm progressivamente mais classificadas conforme aliases vão sendo aprendidos.
- **Contagem no Config:** `renderConfigCounts()` mostra dinamicamente "X lotes · Y importações antigas pra revisar" (em vermelho se há órfãos), pra Tiago saber que tem coisa pendente sem precisar abrir.

**Validação:** simulação Node com 3 gastos órfãos + 1 entrada órfã + 1 gasto manual + 1 entrada manual antiga. Migração agrupou os 4 órfãos num lote. Undo removeu só os 4 e preservou os 2 manuais. ✓ 4/4 OK.

**Falha exposta no segundo uso real (C):** Tiago subiu o extrato outra vez. Notou que Eleva (R$ 6.283,27) e Wise/Hansard (R$ 2.585,35) — gastos que ele já tinha lançado manualmente — apareceram como novos, não como duplicatas. Causa: meu dedup só procurava na MESMA conta com janela apertada (`g.contaId === contaId` + ±0,01 + ±2 dias). Mas os gastos manuais antigos podem estar sem `contaId` (cadastrados antes do sistema de contas existir), ou em outra conta (Wise como invest em conta de investimentos, não na Itaú). Além disso, Tiago não podia conferir os gastos antigos pra decidir caso a caso porque a sheet de revisão bloqueia o resto do app (modal). E o único controle por linha era incluir/ignorar — sem como classificar inline antes de aplicar.

**(C) v0.23.2 — Revisão inteligente: dedup em 3 níveis + classificação inline + modo rascunho**

3 fixes estruturais na tela de revisão:

- **Dedup em 3 níveis de confiança** (substitui `dedupExtrato` simples):
  - **ALTA:** mesma conta, valor ±R$ 0,01, ±2 dias — banner vermelho, default ignorar.
  - **MÉDIA:** qualquer conta (ignora `contaId`), valor ±R$ 0,01, ±7 dias — banner amarelo, default ignorar. Pega Eleva (manual sem contaId), Wise (em conta diferente).
  - **BAIXA:** valor ±1%, ±15 dias, palavra-chave ≥4 chars compartilhada no nome — banner cinza, default INCLUIR (Tiago decide). Pega casos onde o valor não bate exato (juros adicionados, tarifa anexa).
  - Não exclui silenciosamente como antes — anota `lanc.potencialMatch = {id, nome, data, valor, categoria, subcategoria, confianca, kind}` e renderiza inline na linha pra Tiago decidir.

- **Modo rascunho persistido em localStorage:**
  - Chave `financa.revisaoExtratoDraft` guarda `revisaoExtratoState` serializado + timestamp.
  - Novo botão "salvar e voltar depois" no rodapé da sheet — fecha sem aplicar, mantém progresso.
  - `abrirSeletorExtrato()` checa `temRascunhoRevisao()` antes de abrir o file picker; se há rascunho, abre modal `askChoice` com 3 opções: retomar / descartar / cancelar.
  - Helper `askChoice({title, message, options})` (novo) — variante de `askConfirm` com N botões.

- **Classificação inline por linha:**
  - Cada saída ganha botão tracejado "`Tiago > a classificar  ✎ editar`".
  - Toque abre modal sheet `_rexAbrirPickerClassificacao(idx)` com chips de categoria (5 fixas com cor do membro) + select de subcategoria (subcatsExclusivas + subcatsUniversais combinadas) + select opcional de grupo (filtra `state.grupos[cat]` não-arquivados).
  - Mudar categoria reseta subcategoria se ela não pertence à nova categoria.
  - Salvar marca `l.classifManual = true` + `l.classifManualCategoria/Subcategoria/GrupoId`.
  - Badge "✓ classificado" aparece na linha.
  - Auto-save no rascunho a cada classificação.

- **Aliases aprendidos retroativamente:** quando Tiago aplica o lote, cada gasto com `l.classifManual` chama `aprenderAlias(descricao, categoria, subcategoria, grupoId)`. Próximo extrato Itaú já traz GILSON, MARINA, etc. pré-classificados.

- **Filtros novos:** "⚠ possíveis duplicatas (N)" e "❓ a classificar (N)" — Tiago foca no que importa.

**Validação:** simulação Node com 4 casos do mundo real (Eleva, Wise, Marina, Sthefan novo). 3/4 detectados como duplicata com confiança correta (média/média/alta), 1 corretamente identificado como novo. ✓

**(D) v0.24.0 — Fatia 5c · Relatório Detalhado por escopo**

**Decisão estratégica:** pausar Etapa 5b por aqui (Itaú importa, undo funciona, classificação inline). Sicoob e BTG (5b.3 e 5b.4) são repetições de trabalho com layouts diferentes — podem esperar. O Tiago pediu uma nova funcionalidade que está mais alinhada com o objetivo final do projeto ("relatórios dignos de auditor fiscal profissional"): listar gastos individuais, não só agregados.

**Implementação:** 4ª aba "Detalhe" em `REL_TABS` (junto com Rel 1, Rel 2, Rel 3). State próprio `rel4State = { ym, periodo, escopo }`. Função `_renderRel4Inner()` orquestra:

- **Chips de escopo no topo:** Família · Casa · Marina · Heitor · Juliana · Tiago. Família soma todas as 5 categorias. Cada chip ganha a cor do membro.
- **Cartão de resumo:** total grande (Fraunces 28px) + contagem de lançamentos + mês legível + top 5 subcategorias com valor e percentual + "outros (N)" agregando o resto.
- **Lista cronológica decrescente** (mais recente primeiro), agrupada por dia com separador "23 de maio".
- **Cada linha mostra:** dia (mono, 28px), nome do gasto, identificação completa (`Categoria > subcategoria > [grupo]` com nome do membro colorido), método de pagamento (🏦 conta nome ou 💳 cartão nome), badges automáticas (🔁 fixo / 💎 invest / 1/12 parcela / 📥 extrato / 📄 fatura / ❓ a classificar), valor em terracota.
- **Tocar na linha** abre `openGastoForm(gid)` pra editar — mesmo fluxo do resto do app.
- **Filtro mestre:** exclui `isAjuste` e `isPagamentoFatura` (são lançamentos técnicos; entrariam como duplicata na auditoria). Inclui `isInvest` mas com badge específico.
- **Exportação PDF:** mesmo botão "📄 PDF" do canto superior direito. `_atualizarPrintMeta()` estendido pra reconhecer rel4 — título do PDF vira "Relatório Detalhado · [Escopo]". `.rel-det-chips-row` adicionada à lista de elementos escondidos no `@media print`. PDF sai limpo, com cabeçalho colorido por escopo e lista tabular.

**Schema sem mudanças.** Tudo é projeção sobre `state.gastos` existente — agnóstico de origem (manual, fatura, extrato).

**Validação:** simulação Node com 8 gastos mistos (5 categorias, 2 meses, 1 isAjuste, 1 isPagamentoFatura, 1 isInvest). Filtros por escopo testados — Família/maio = 5 gastos (não inclui os 2 técnicos), Heitor/maio = 1, Tiago/maio = 2 (inclui invest), Casa/abril = 1. ✓ 4/4 OK.

**Próximo natural:** Fatia 5c.1 (filtros por subcategoria/grupo dentro do escopo + opção de append detalhamento nos Rel 1/2/3) OU retomar Etapa 5b.3 (parser Sicoob) OU pular pra Etapa 6 (generalização comercial / BLUEPRINT-FINANCA).

---

## ⚠️ Sessão anterior (18/maio/2026, noite) — Fatia 5.5 (Exportação PDF) + auditoria de divergência financeira (v0.22.0 → v0.22.2):
...
### Unificação dos relatórios — um botão só (decisão de 21/maio/2026)
O Tiago se incomodou com ter dois botões de PDF (o "📄 PDF rápido" e o "✦ Relatório Elegante"). Decisão: **NÃO** transformar a tela de relatórios em A4 (a tela é ferramenta de trabalho interativa; A4 é ruim no celular e mataria a interatividade). Em vez disso, **fazer o Relatório Elegante crescer** até cobrir tudo que o PDF rápido faz — e então **aposentar o PDF rápido**, deixando um botão só. Já feito nesta direção: fluxo detalhado no Rel 2 (5c.2), gastos por categoria (5c.3) e cronológico (5c.5) no Rel 1/3, com interruptores. Falta pra fechar a unificação: Rel 4 elegante e aposentar o rápido (ver §10).

### Dashboard × Relatório (nota de design — pra versão de empresa)
Distinção registrada a pedido do Tiago: **Dashboard** = painel vivo/interativo (a própria tela de relatórios do app — navega, filtra, atualiza ao vivo), serve pra **monitorar e decidir agora**. **Relatório** = documento estático/arquivável (o Relatório Elegante), serve pra **comunicar e guardar**. É por isso que a tela NÃO vira A4. **Implicação:** a futura versão de empresa precisará de um **dashboard ao vivo** (faturamento do dia, contas a pagar, fluxo projetado) **além** dos relatórios formais (DRE, relatório pro contador) — são necessidades distintas.

### Granularidade do detalhamento por categoria + propósito de cada detalhamento (21/maio/2026)
A seção "gastos por categoria" (5c.3) é, por decisão do Tiago, o **formato PRINCIPAL** do relatório (o que ele usa pra apresentar): membro com **total no topo** → subcategoria com **subtotal** → **itens individuais**. Já a **lista cronológica corrida** (5c.5) é ferramenta secundária, de **conferência/localização** pessoal. Por isso os defaults: toggle "por categoria" **LIGADO**, toggle "cronológico" **DESLIGADO** (liga quando precisa conferir).

---

## 5. Schema do state (atualizado v0.28.0)
...
```js
{
  ...
  entradas: [],     /* {id, fonteId, contaId, valor, data, ym, observacao,
                          origem, loteImportacaoId} */   /* origem/lote: v0.23.x */
  ...
  gastos: [],       /* {
                       ...
                       origem,                  // v0.23.x — 'fatura' | 'extrato'
                       loteImportacaoId         // v0.23.1 — FK pro lote de importação
                     } */

  transferencias: [],

  lotesImportacao: [],  /* v0.23.1 — {id, tipo, contaId, contaNome, dataImport,
                            descricao, gastoIds:[], entradaIds:[], quantidade}.
                            Habilita desfazer uma importação inteira em lote. */
...
- **v0.23.1** — `lotesImportacao[]` + `gasto.loteImportacaoId`/`origem`. Migração retroativa idempotente agrupa gastos órfãos `origem:'extrato'` num lote de "🛟 Recuperação".
- **v0.27.0** — fluxo de extrato passou a alimentar `aliasItems[].identificacao` (o campo existia desde v0.11.0 mas não era preenchido na importação de extrato).
- **v0.28.0** — sem mudança no state persistido. Os campos de classificação manual do extrato (`classifManual*`, incluindo o novo `classifManualFixo`) vivem no `revisaoExtratoState` temporário, não no documento salvo.
...
## 10. Etapas pendentes (roadmap atualizado v21)

> **Já entregue desde o roadmap original (NÃO confundir com pendente):** Etapa 5 completa (5.1·5.2·5.3·5.5 ✅, 5.4 dispensada por design) — **ETAPA 5 FECHADA em 21/maio/2026**; Etapa 5c (5c·5c.1·**5c.2·5c.3·5c.4·5c.5** ✅); Etapa 5b parsers de extrato **Itaú ✅ e Sicoob ✅** (BTG pendente); **Fatia 5d** — Relatório Elegante (v0.26.0) ✅; **Fatia 5e** — extrato cronológico + classificar com nome/subcategoria (v0.27.0) ✅; **Fatia 5e.1** — paridade do picker de extrato (v0.28.0) ✅. O que segue abaixo é o que REALMENTE falta.

### ✅ Etapa 5 — Relatórios (5.1·5.2·5.3·5.5 entregues · 5.4 dispensada — ETAPA COMPLETA)
...
### ✅ Etapa 5c — Relatório Detalhado + unificação de relatórios (5c·5c.1·5c.2·5c.3·5c.4·5c.5 ✅)

- **Fatia 5c ✅ Relatório Detalhado** (v0.24.0) — 4ª aba "Detalhe", lista cronológica por escopo.
- **Fatia 5c.1 ✅ Filtros no Detalhe + lista nos PDFs** (v0.25.0) — filtros subcat/grupo no Rel 4; apêndice cronológico anexável aos PDFs do Rel 1 e Rel 3 (reconciliado por construção com o hero).
- **Fatia 5c.2 ✅ Fluxo de caixa detalhado no Rel 2 Elegante** (v0.30.0) — redirecionada (decisão de unificação): em vez de anexar a lista ao PDF rápido, foi pro **Relatório Elegante**. Lista cronológica entradas/saídas/invest com rodapé reconciliando com a capa.
- **Fatia 5c.3 ✅ Gastos por categoria no Rel 1 Elegante** (v0.31.0) — membro (total) → subcategoria (subtotal) → itens. Formato principal do relatório.
- **Fatia 5c.4 ✅ Caixa de opções do Rel 1 + toggle de categoria** (v0.32.0) — 3 toggles numa caixa; `relPdfDetalheCategoria` (default ON).
- **Fatia 5c.5 ✅ Cronológico no Rel 1/3 Elegante** (v0.33.0) — controlado pelo toggle "lista detalhada" (default OFF). Coerência: o que liga é o que sai.

**⏳ Unificação de relatórios — passos finais (backlog pós-Etapa 5):** o Relatório Elegante já cobre Rel 1/2/3 com detalhamentos controláveis. Pra fechar a unificação num botão só (ver decisão em §4): (a) **dar Relatório Elegante ao Rel 4**; (b) **aposentar o "📄 PDF rápido"** (só depois de (a), pois o Rel 4 ainda depende dele); (c) **caixa de toggles própria no Rel 3** (hoje obedece os interruptores ligados no Rel 1 — funciona via prefs globais, mas sem controles visíveis no corpo do Rel 3). NOTA: "levar detalhamentos pra tela" foi **dispensado** (o app já tem extrato do mês + drill nas categorias).

### ⏳ Etapa 5b — Importação de extrato bancário (gap-filler) — Itaú ✅, Sicoob ✅, BTG pendente

**Modelo:** extrato como gap-filler, não fonte primária. Preenche gaps onde Tiago não registrou manualmente, sem gerar duplicatas. **Etapa 4 pronta** → 5b reconhece pagamentos de fatura no extrato e não os duplica (já existem como saídas técnicas com `isPagamentoFatura: true`).

**Estado:** pipeline + parser **Itaú** (v0.23.x) e parser **Sicoob** (v0.29.0) entregues — com undo em lote, dedup em 3 níveis, modo rascunho e classificação inline. O Sicoob fechou a conciliação de saldo em R$ 0,00 no extrato de maio. Pendente: **5b.4 parser de extrato BTG**.
...
**Pré-requisito de 5b.4:** 1 extrato de **conta corrente** real do BTG. Atenção: o PDF BTG no projeto é **fatura de cartão** (já tem parser próprio na Fatia 3.3) — extrato de conta é layout diferente. O Sicoob (5b.3) já foi destravado com extrato real e está pronto.

**Layout do extrato Sicoob (referência, 5b.3):** cada movimento numa linha `DD/MM HISTÓRICO VALOR{D|C|*}` (data sem ano — vem do PERÍODO no header; sufixo D=débito/saída, C=crédito/entrada, *=saldo/ignora); descrições em linhas seguintes sem data (estabelecimento/CPF/DOC); seção RESUMO+LANÇAMENTOS FUTUROS cortada. Classificação por `l.historico` (campo exclusivo do parser Sicoob; Itaú não tem). Pagamento de fatura = DÉB.CONV.DEM.EMPRES + DOC MASTERCARD.
...
- **v17** (19–20 maio 2026) — **Etapa 5b (importação de extrato — Itaú) + Fatia 5c (Relatório Detalhado) ENTREGUES — 4 versões em sequência (v0.23.0 → v0.24.0)**. Sessão longa e iterativa atravessando 2 etapas. **(A) v0.23.0 — Etapa 5b · Fatias 5b.1 + 5b.2 bundled**: pipeline de importação de extrato bancário com parser Itaú dedicado, arquitetura paralela à de fatura (não substitui, coexiste); novos `detectarBancoExtrato`, `parseExtrato` (dispatcher), `parserExtratoItau`, `classificarLancamentosExtrato`, `dedupExtrato`, `openRevisaoExtratoSheet`, `aplicarLancamentosExtratoImportados`; classifica em 5 tipos (`PAGAMENTO_FATURA` com match contra `isPagamentoFatura`, `ENTRADA_RENDA`, `ENTRADA_RENDIMENTO`, `SAIDA_CONSUMO`, `SAIDA_TARIFA`, `IGNORAR`); parser Itaú validado em 11/11 checks; single-source-of-truth da versão via `#appVersionTag` no rodapé do Config. **(B) v0.23.1 — Sistema de lote + undo + pré-classificação por alias**: schema novo `state.lotesImportacao[]` com FK reversa (`gasto.loteImportacaoId`); migração retroativa idempotente agrupa órfãos `origem:extrato` num lote de "🛟 Recuperação"; `desfazerLoteImportacao(loteId)` remove records e o lote; nova UI no Config "📥 Histórico de importações"; confirmação modal antes de aplicar; reuso de `buscarAliasParaDescricao()` no parser de extrato. **(C) v0.23.2 — Revisão inteligente em 3 níveis**: dedup substituído por detecção em 3 níveis de confiança (alta = mesma conta ±0,01 ±2 dias; média = qualquer conta ±0,01 ±7 dias; baixa = ±1% ±15 dias + palavra-chave); banner inline com info do potencial match ao invés de exclusão silenciosa; modo rascunho persistido em `localStorage` (chave `financa.revisaoExtratoDraft`); novo botão "salvar e voltar depois"; `abrirSeletorExtrato` checa rascunho antes; classificação inline por linha via `_rexAbrirPickerClassificacao` (modal sheet com chips de categoria + select de subcategoria + select de grupo); aliases aprendidos retroativamente quando Tiago classifica. **(D) v0.24.0 — Fatia 5c · Relatório Detalhado**: 4ª aba "Detalhe" em REL_TABS; `rel4State = {ym, periodo, escopo}`; chips de escopo (Família/Casa/Marina/Heitor/Juliana/Tiago) com cor do membro; cartão de resumo com total + contagem + top 5 subcategorias com %; lista cronológica decrescente agrupada por dia com separadores; cada linha mostra dia/nome/categoria›subcategoria›grupo/método/badges (🔁 fixo · 💎 invest · N/M parcela · 📥 extrato · 📄 fatura · ❓ a classificar)/valor; toque abre `openGastoForm` pra editar; exclui `isAjuste` e `isPagamentoFatura` automaticamente; `_atualizarPrintMeta` estendido pra rel4 ("Relatório Detalhado · Escopo" no header do PDF); `.rel-det-chips-row` escondido em `@media print`. Schema sem mudanças. **6 lições novas em NOTAS-COMERCIAIS:** (58) lote de importação como FK reversa habilita undo; (59) dedup em 3 níveis de confiança vs exclusão binária; (60) modo rascunho persistido resolve modal que bloqueia o resto do app; (61) classificação inline durante revisão vs apply-then-edit; (62) aliases aprendidos retroativamente fecham o loop; (63) lista cronológica como complemento a agregados — fato vs métrica. 16.972 linhas no `index.html` (+1.894 desde v0.22.2). 63 itens em NOTAS-COMERCIAIS. **Próximo: Fatia 5c.1 (filtros + append em Rel 1/2/3) OU Etapa 5b.3 (parser Sicoob) OU Etapa 6 (BLUEPRINT-FINANCA / generalização comercial).**
- **v18** (20 maio 2026) — **Fatia 5c.1 entregue (v0.25.0): filtros no Relatório Detalhado + lista detalhada anexável aos PDFs.** Escolhida sobre 5b.3 (Sicoob — sem extrato de conta) e Etapa 6 (generalização prematura). **(5c.1.a)** `rel4State.filtroSubcats[]` + `filtroGrupos[]`; helpers `_rel4AplicarFiltros` (interseção subcat ∩ grupo) e `_rel4Catalogo` (opções de chip a partir da base não-filtrada); seção "filtrar" no header (chips multi-seleção; subcat com ≥2, grupo se houver); resumo recalcula sobre o filtrado; botão limpar; reset ao trocar escopo; recorte vai pro título do PDF. **(5c.1.b)** toggle "📎 incluir lista detalhada no PDF" (Rel 1/Rel 3, preferência em `localStorage` `financa.relPdfApendice`); render cronológico extraído em `_relListaCronologicaHtml(gastos)` reusado por tela e PDF; `_relApendiceGastos(viewId)` usa o MESMO conjunto+filtro do hero (`relCategoriasIncluidas`/`REL3_CATS_TIAGO` + `_relGastoSaida`) respeitando o período; `_montarApendicePDF`/`_limparApendicePDF` injetam/removem `#relApendice` (invisível em tela, revelado em `@media print` com `page-break-before:always`), limpeza no `afterprint`. **Decisão de escopo:** append do Rel 2 isolado como **Fatia 5c.2** (regime de caixa com entradas, estrutura diferente — renderer próprio). **Bônus:** ativada a variável CSS `--chip-color` órfã (chip de escopo ativo ganha destaque). Validação: 4 checks + simulação Node 21/21 (filtros + invariante apêndice==hero com Tiago OFF/ON e multi-mês). Schema sem mudanças. 6 lições novas: (64) render parametrizado como fonte única tela/PDF; (65) reconciliação por construção (detalhe usa mesmo conjunto+filtro do agregado); (66) apêndice respeita escopo temporal do relatório, não o hero; (67) filtro multidimensional como interseção + base alimenta catálogo; (68) token CSS órfão — auditar variáveis sem consumidor; (69) fatia fina protege o componente mais delicado. 69 itens em NOTAS-COMERCIAIS. **Próximo: Fatia 5c.2 (append de fluxo no Rel 2) OU Etapa 5b.3 (parser de extrato Sicoob, requer extrato de conta) OU Etapa 6 (BLUEPRINT-FINANCA).**
- **v19** (20 maio 2026, noite) — **Fatias 5d + 5e entregues (v0.26.0 -> v0.27.0).** (5d/v0.26.0) Relatorio Elegante: namespace `window.relatorioElegante` gera documento HTML autonomo (Montserrat, capa navy, graficos SVG, layout A4) em nova aba, com toolbar (imprimir/salvar PDF + baixar HTML) e fallback de download quando o popup e bloqueado no iOS; abrange Rel 1, Rel 2 e Rel 3 (Rel 4 fora); botao "Relatorio" navy ao lado do "PDF". Chama os MESMOS motores de calculo das telas -> reconcilia por construcao; 33/33 testes de reconciliacao. (5e/v0.27.0) Extrato cronologico na tela (botao "Ver extrato do mes" em Familia/Tiago, sheet com month-nav + busca livre + lista via `_relListaCronologicaHtml`) + picker de classificacao do extrato com nome editavel e "+nova" subcategoria inline (modo `criandoSub`, evita modal empilhado), nome amigavel alimentando `aliasItems[].identificacao`. NOTAS-COMERCIAIS em 75 itens. **Falha de documentacao:** estas duas entregas atualizaram o STATUS no topo do prompt mas NAO ganharam entrada no historico nem atualizacao do roadmap — origem da defasagem corrigida na v20.
- **v20** (20 maio 2026, continuacao) — **Fatia 5e.1 entregue (v0.28.0) + consolidacao geral deste prompt.** (5e.1) Paridade do picker de classificacao do extrato com o da fatura: secao "grupo" sempre visivel + "+novo grupo" inline (estado `criandoGrupo`), toggle "marcar como fixo" (`pickerFixo` -> `classifManualFixo` -> aplicado em `isFixo` e ensinado ao alias). Compatibilidade retroativa garantida (`=== true`). Validacao: 5 checks + IDs<->handlers + Node 3/3. Origem: Tiago relatou em uso real que faltavam campos de edicao no EXTRATO (nao na fatura, como a nota errada do prompt dizia) — auditoria do codigo confirmou e redirecionou. (Consolidacao) sincronizado topo<->historico<->roadmap<->schema; adicionado bloco "LEIA PRIMEIRO" (re-inteiracao barata de chats novos); registrados v0.26/v0.27/v0.28; carimbo do schema atualizado pra v0.28.0 com `lotesImportacao`/`loteImportacaoId`/`origem`. Licoes de processo: (1) atualizar o prompt a cada fatia, nao deixar acumular; (2) o codigo e a fonte de verdade quando documentacao e comportamento divergem; (3) separar "documento de continuidade" (este prompt) de "documento reconstrutor" (BLUEPRINT, etapa final) de "verdade executavel" (index.html).
- **v21** (21 maio 2026) — **Unificacao dos relatorios + parser de extrato Sicoob — 5 fatias (v0.29.0 -> v0.33.0); ETAPA 5 FECHADA.** **(5b.3/v0.29.0)** Parser de extrato de conta Sicoob: `parserExtratoSicoob` + `_sicoobDataPdfParaISO` no dispatcher `parseExtrato`; layout `DD/MM HISTORICO VALOR{D|C|*}` (ano vem do PERIODO no header; sufixo D=saida/C=entrada/*=saldo), descricoes multi-linha, corte da secao RESUMO+LANCAMENTOS FUTUROS; classificacao por bloco Sicoob guardado por `if (l.historico)` (campo exclusivo — nao toca o Itau); pagamento de fatura = DEB.CONV.DEM.EMPRES + DOC MASTERCARD (anti-duplicidade, casa `_matchPagamentoFatura`). **Audit de ouro: conciliacao de saldo fechou em R$ 0,00** (32 lancamentos de maio). **(5c.2/v0.30.0)** Fluxo de caixa detalhado no Rel 2 Elegante — redirecionada da ideia original (PDF rapido) pro Elegante por decisao de unificacao; lista cronologica entradas(verde)/saidas(vermelho)/invest(azul) reconciliando com a capa. **(5c.3/v0.31.0)** Gastos por categoria no Rel 1 Elegante: membro(total no topo)->subcategoria(subtotal)->itens. **(5c.4/v0.32.0)** Caixa unica com 3 toggles empilhados no Rel 1 + nova pref `relPdfDetalheCategoria` (localStorage, default ON — desliga nos periodos longos). **(5c.5/v0.33.0)** Cronologico no Rel 1/3 Elegante, controlado pelo toggle "lista detalhada" (`relPdfApendice`, default OFF) — que agora vale no rapido E no Elegante. **Etapa 5 declarada FECHADA**; refinamentos viraram backlog (Rel 4 elegante, aposentar o PDF rapido, caixa de toggles propria no Rel 3); "detalhamentos na tela" dispensado (app ja tem extrato do mes + drill). Decisoes de design em §4: unificacao num botao so; **dashboard x relatorio** (orienta a versao de empresa); granularidade e proposito dos detalhamentos (por categoria = principal/ON; cronologico = conferencia/OFF). **Licoes de metodo:** validar parser ISOLADO em Node contra o texto real do PDF ANTES de transplantar pro arquivo de ~18k linhas, depois validar o transplante (extrair as funcoes do index e rodar); a conciliacao de saldo (anterior+creditos-debitos=final) e o audit de ouro do extrato — R$ 0,00 prova que nada foi perdido nem inventado; guarda por campo exclusivo (`l.historico`) estende o classificador sem regressao no banco existente; reconciliacao por construcao (coletar as listas detalhadas das MESMAS fontes que geram os totais da capa); a duvida do usuario pode redirecionar uma fatia pra melhor (5c.2 saiu do PDF rapido pro Elegante); cor inline vence a especificidade de classe (`.tg-val` e terracota por default). **Proximo: backlog da unificacao (Rel 4 elegante -> aposentar o PDF rapido) OU Etapa 5b.4 (extrato BTG, requer extrato de conta real) OU Etapa 6 (BLUEPRINT-FINANCA / versao de empresa).**
